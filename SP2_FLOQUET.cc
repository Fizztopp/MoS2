...
//#define Ax_peierls     0.5                                              // Amplitude of Applied Field in x-direction
//#define Ay_peierls     0.5                                              // Amplitude of Applied Field in y-direction
...
#define timesteps_F 400                                                 // # of steps to perform integration over one period T=2pi/Omega                             
		...
		{	
			for(int m=0; m<3; ++m)
			{
				for(int n=0; n<3; ++n)
				{
	                ...
	                if(d<=3.30)
	                {
						Hk[fq(i,j,NATOM)] += t1/rd1*exp(qq1/rg*(1.-(d/aa1)))*exp(II*(kx*rx+ky*ry))*exp(-II*(Ax_t(time)*rx+Ay_t(time)*ry));      	
					}		       
				}
			}
			Hk[fq(j,i,NATOM)] = conj(Hk[fq(i,j,NATOM)]);	
		...
		for(int j=NATOM/2; j<NATOM; ++j)
		{
			for(int m=0; m<3; ++m)
			{
				for(int n=0; n<3; ++n)
				{
	                ...
	                d = sqrt(pow(rx,2.)+pow(ry,2.)+pow(rz,2.));   
					// Vpp_pi term
					// Hk[fq(i,j,NATOM)] += (1.-pow(aa2/d,2.))*t1/rg*exp(qq1/rg*(1.-(d/aa1)))*exp(II*(kx*rx+ky*ry))*exp(-II*(Ax_t(time)*rx+Ay_t(time)*ry+Az_t(time)*rz));
					// Vpp_sigma term       
					if(d<=7.50)
	                {
						Hk[fq(i,j,NATOM)] += t2/rd2*exp(qq2*(1.-(d/aa2)))*exp(II*(kx*rx+ky*ry))*exp(-II*(Ax_t(time)*rx+Ay_t(time)*ry+Az_t(time)*rz));  
					}	         
				}
			}
			Hk[fq(j,i,NATOM)] = conj(Hk[fq(i,j,NATOM)]);
...
void Hk_bands_Floquet(dvec &BANDS_FLOQUET, dvec &OVERLAP_FLOQUET, dvec &OVERLAP_FLOQUET_s, dvec &OVERLAP_FLOQUET_p, cvec &Hk_FLOQUET, dvec &evals_FLOQUET, vector<dvec> &K_PATH, vector<dvec> &UNIT_CELL, const dvec &lvec, int &numprocs, int &myrank)
/**
 ...
 */
{
	...
	#pragma omp parallel for collapse(4)                                
		...
		// Diagonalize Floquet matrix in order to get eigenvalues and eigenvectors		
		...
		// Calculate squared overlap of Floquet eigenstates with eigenstates of eq. Hamiltonian / s-,p-weighted overlap
		// calculate bands of unperturbed Hamiltonian
		set_Hk0(K_PATH[k], H0, lvec, UNIT_CELL);
		diagonalize(H0, evals);
		for(int i=0; i<NATOM*(2*n_max+1); ++i) // select Floquet band
		{
			temp = 0.;
			temp_orb_s = 0.;
			temp_orb_p = 0.;
			// Overlap of H(0,0) bands with original bands
			for(int w=0; w<NATOM; ++w) // select original band
			{
				tempc = 0.;
				for(int j=0; j<NATOM; ++j)
				{
					tempc += Hk_FLOQUET[fq(i,NATOM*n_max+j,NATOM*(2*n_max+1))]*conj(H0[fq(w,j,NATOM)]);
				}
				temp += real(tempc*conj(tempc));
				//~ // Squared overlap with s-bands
				//~ if(NATOM-3<w && w<NATOM)
				//~ {
					//~ temp_orb_s += real(tempc*conj(tempc));
				//~ }
				//~ // Squared overlap with p-bands
				//~ if(NATOM-7<w && w<NATOM-2)	
				//~ {	
					//~ temp_orb_p += real(tempc*conj(tempc));
				//~ }
			}
			...
			for(int m=-m_max; m<m_max+1; ++m) // select Floquet sector
			{
				...
				for(int jj=0; jj<NATOM; ++jj) // select atomic position for calulcation pf p/s overlap
				{
					...
					{
						Cs += conj(Hk_FLOQUET[fq(i,NATOM*(m+m_max)+jj,NATOM*(2*m_max+1))])*1.0;                       // symmetric weight
						Cx += conj(Hk_FLOQUET[fq(i,NATOM*(m+m_max)+jj,NATOM*(2*m_max+1))])*rx/sqrt(rx*rx+ry*ry);      // antisymemtric (px) weight
						Cy += conj(Hk_FLOQUET[fq(i,NATOM*(m+m_max)+jj,NATOM*(2*m_max+1))])*ry/sqrt(rx*rx+ry*ry);      // antisymemtric (py) weight 
					}
				}	
			...
			OVERLAP_FLOQUET_s[fq(k,i,NATOM*(2*n_max+1))] = temp_orb_s; 
			OVERLAP_FLOQUET_p[fq(k,i,NATOM*(2*n_max+1))] = temp_orb_p; 
		}	  				
	}
	delete TEMP1, TEMP2;	
		...
		MPI_Allreduce(MPI_IN_PLACE, &OVERLAP_FLOQUET_s[0], NATOM*(2*n_max+1)*num_kpoints_path, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		MPI_Allreduce(MPI_IN_PLACE, &OVERLAP_FLOQUET_p[0], NATOM*(2*n_max+1)*num_kpoints_path, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
	// Store data
	int count = COUNT;
	if(myrank==0)
	...
	if(myrank==0)
	{
		...
		if (myfile.is_open())
		{
			for(int k=0; k<num_kpoints_path; k++)
			{
				for(int m=0; m<NATOM*(2*n_max+1); m++)
				{
					myfile << OVERLAP_FLOQUET_s[fq(k,m,NATOM*(2*n_max+1))] << " " ;
				}
			myfile  << endl;
			}
		myfile.close();
	...
	if(myrank==0)
	{
		...
		if (myfile.is_open())
		{
			for(int k=0; k<num_kpoints_path; k++)
			{
				for(int m=0; m<NATOM*(2*n_max+1); m++)
				{
					myfile << OVERLAP_FLOQUET_p[fq(k,m,NATOM*(2*n_max+1))] << " " ;
				}
			myfile  << endl;
			}
		myfile.close();
...
#pragma omp parallel 
	...
	// Triangular recriprocal lattice vectors
	//const double g0 = 4.0*PI/(sqrt(3.)*d);
	//const dvec gvec = {g0*cos(angle1-PI/6.),  g0*sin(angle1-PI/6.),  g0*cos(angle1+PI/2.), g0*sin(angle1+PI/2.), g0*cos(angle1+PI/6.), g0*sin(angle1+PI/6.)};
	...
	dvec *OVERLAP_FLOQUET_s = new dvec(num_kpoints_PATH*NATOM*(2*n_max+1));
	dvec *OVERLAP_FLOQUET_p = new dvec(num_kpoints_PATH*NATOM*(2*n_max+1));
	...
	//groundstate(Hk[0], evals, kweights_full, BZ_IRR, UNIT_CELL, lvec, mu, numprocs, myrank);
	...
	//EQ_BC_LOOP_PATH(-1e-5, +1e-5, 2, Hk[0], lvec, UNIT_CELL, BZ_FULL, evals, bands_BCs, numprocs, myrank);

	if(myrank==0){cout << "Start caluclation of Floqeut bands along k-path" << endl;}		
	Hk_bands_Floquet(BANDS_FLOQUET[0], OVERLAP_FLOQUET[0], OVERLAP_FLOQUET_s[0], OVERLAP_FLOQUET_p[0], Hk_FLOQUET[0], evals_FLOQUET[0], K_PATH, UNIT_CELL, lvec, numprocs, myrank);
	...
	//FLOQUET_BC_LOOP_PATH( -1e-5, +1e-5, 2, lvec, UNIT_CELL, BZ_FULL, evals_FLOQUET[0], bands_BCs_FLOQUET, numprocs, myrank);
